...
<style type="text/css">
	...
	.preloader{ position:absolute; left:0; width:100%; height:100%; background:#212832; text-align:center; padding-top:12%; z-index: 100; vertical-align: center; opacity: 1;}
</style>
...
<div class="preloader" id="preloader"><img src="{{asset('assets/img/904 .gif')}}" /></div>
<div class="content">
					...
					<div class="tab-content" id="v-pills-zilliqua-btc-tabContent">
						...
						<div class="tab-pane fade" id="v-pills-zilliqua-btc-withdrawl" role="tabpanel" aria-labelledby="v-pills-zilliqua-btc-withdrawl-tab">
					  	...
					  	<div class="tab-pane show active table-responsive" id="v-pills-zilliqua-btc-history" role="tabpanel" aria-labelledby="v-pills-zilliqua-btc-history-tab">
					  		<h4 class="crypt-down pb-3" style="font-size: 24px; font-weight: 400"><i class="la la-clock-o"></i>&nbsp; User Transaction History</h4>
							  		...
							  		@if($data)
								  		...
								  		@foreach($data as $item)
										    ...
										    <tr>
										      <td scope="row">{{ $i }}</td>
										      <td scope="row">
										      	<img src="{{asset($item->profile_image)}}" style="width: 40px; height:40px" class="img-circle">
										      </td>
										      <td scope="row"><a href="{{ url('/history-user/'.$item->id) }}">
										      	<i class="la la-edit"></i>&nbsp;{{ $item->name }}</a></td>

										      <td scope="row">{{ $item->email }}</td>
										      <td scope="row">{{ $item->pay_address }}</td>
										      ...
										      <td class="crypt-down"><i class="far fa-comments"></i><a href="{{route('go_chat')}}">&nbsp;Chat</a></td>
										      
										      <td class="crypt-down"><a href="{{ url('/history-del/'.$item->id) }}">
										      	<i class="la la-remove">&nbsp;</i>Delete</a></td>
